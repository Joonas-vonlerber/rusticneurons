...
pub fn initialize_expected_outputs_mnist(labels: &Vec<u8>) -> Vec<vector<f32>> {
...
pub fn loss_mnist(
    ...
    test_data: &Vec<vector<f32>>,
    test_expected_outputs: &Vec<vector<f32>>,
) -> (f64, f32) {
    let mut forward: vector<f32>;
    let mut loss: f64 = 0.0;
    let mut got_right: u16 = 0;
    for (input, expected) in test_data.iter().zip(test_expected_outputs.iter()) {
...
pub fn print_number_and_test(neural_network: &mut NeuralNetwork) {
    ...
    let foo = files::load_inputs(r"resources/train-images-idx3-ubyte").unwrap();
    ...
    println!("{}", image);
    println!("{}", neural_network.forward_phase(&foo[n].clone()).outputs);
    println!(
        "{}",
        neural_network.forward_phase(&foo[n].clone()).outputs.imax()
    );
}
