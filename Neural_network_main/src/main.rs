...
fn main() {
    ...
    // );
    let mut neural_network: NeuralNetwork = load_network_bincode("networks/MNIST_Adam").unwrap();
    ...
    println!("{:?}", loss_mnist(&mut neural_network, &test_data));
    neural_network = neural_network.train(&mut data, 0.0001, 100);
    println!("{:?}", loss_mnist(&mut neural_network, &test_data));
